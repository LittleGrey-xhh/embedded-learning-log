...
### 知识点：
#### static
1.在函数内定义的静态局部变量，如static int a = 0;  多次调用函数，只在第一次调用时初始化为0
2.只改变生命周期，不改变作用域
3.函数执行结束后，变量的值不会丢失，下次调用该函数时，会保留上次的值。
4.当修饰全局变量时，将全局变量的作用域限制在当前源文件（.c文件）内，其他源文件即使使用 extern 关键字，也无法访问这个变量，实现"文件级私有"，避免不同文件中的全局变量命名冲突。

#### 内存对齐
定义：在64位系统中，CPU在访问内存的时候，一次一般访问8个字节的空间
内存对齐要求变量的内存地址能够被该变量的大小整除，地址值必须是变量大小的整数倍这样可以确保计算机一次就能把内存中的数据读出来，并且不用做数据拼接

作用：提高CPU访问内存的效率，牺牲内存为代价

原则：结构体变量存储的时候，同样遵循内存对齐的原则
确定变量位置：只能放在自已类型整数倍的内存地址上
最后一个补位：结构体的总大小，是最大类型的整数倍

注：补位的目的是：为下一个同类对象留出对齐空间，比如说结构体数组


2026-07-24 C语言数组、指针与结构体的常见误区辨析
知识点
结构体类型定义中不能包含可执行语句，也不能在C语言中给成员设置默认值（C++11起支持）。

对结构体变量的字符数组成员，不能用 = 直接赋值，因为数组名是地址常量，应使用 strcpy。

二维数组名在表达式中退化为指向首行（一维数组）的指针，类型为 int (*)[列数]。

*(*(a+1)+2) 等价于 a[1][2]，分解步骤：a+1 指向第1行，*(a+1) 得到第1行（退化为int*），+2 偏移到该行第2列，最后解引用得到元素值。

sizeof(数组名) 返回整个数组占用的字节数，不会退化为指针大小。

char *str[] 是指针数组：str 先与 [] 结合成为数组，元素类型为 char*。若改为 char str[][10] 则是二维字符数组，内容直接存储在数组中。

数组指针与指针数组的区别：声明时优先级 [] 高于 *，无括号时 [] 先结合形成数组；有括号如 int (*p)[3] 则 * 先结合形成指针。

p[2] 恒等于 *(p+2)，无论 p 是静态数组名还是动态分配的指针，这是C标准定义的下标运算。

踩过的坑
错误直觉：以为在结构体类型里面可以给成员直接赋值，或者在变量处用 a1.name = "abc" 给字符数组成员赋值。

实际发生：结构体类型定义中不能出现语句；对字符数组成员，数组名不能被赋值，需要用 strcpy。

混淆数组元素是指针与整个数组是指针：误以为 char *str[] 中的 str 是指针，实际上它是数组，在 sizeof 中能证明——32位下 sizeof(str)=16 是整个数组大小，不是单个指针的大小。

指针数组 vs 数组指针 概念混淆：凭直觉记不清谁是谁。

澄清：看最后一个词——"指针数组"的本体是数组，"数组指针"的本体是指针。声明时先结合谁就是什么：char *str[]，str 先与 [] 结合，所以是数组。
...
struct student a1;
strcpy(a1.name, "abc");   // 正确：使用 strcpy
核心思路：数组名不能作赋值左值，必须用 strcpy 或逐个字符拷贝。
...
核心思路：a 是指向含3个int的数组的指针，a+1 跳一行，*(a+1) 得到该行并退化为指向首元素的指针，+2 移动至列元素，解引用取值。
...
int (*p)[3];                                     // 数组指针：指针，指向含3个int的数组
核心思路：关注优先级——没括号先数组后指针（指针数组），有括号先指针后数组（数组指针）。
...
p[2] = 10;           // 完全等价于 *(p + 2) = 10;
核心思路：指针支持 [] 运算符，p[i] 编译器会转换为 *(p + i)，与静态数组行为一致。

关键词
结构体数组成员赋值 二维数组指针运算 sizeof指针数组 指针数组 数组指针 malloc下标等价 *(a+1)+2 数组名退化
