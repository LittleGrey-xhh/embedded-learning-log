...
# 8.31 笔记：计算机基础 · 进程与线程 · 进程生命周期

## 1. 存储体系

### 1.1 外存储器
- 定义：硬盘、U 盘、光盘等，断电后数据不丢失（永久存储）
- 最小存储单元：**数据块（Block）**
- 每个数据块由 n 个扇区（Sector）组成，读写以块为单位

### 1.2 内存储器
- 定义：内存（RAM），断电后数据丢失
- 最小存储单元：**字节（Byte）**，按字节寻址
...
- 控制器：取指令、指挥各部件协同工作
- 运算器：算术 / 逻辑运算
- 寄存器：存放 **CPU 正在执行的指令和数据**（访问频率最高）
- 缓存（Cache）：存放**访问频率较高**的数据，介于寄存器与内存之间

存储速度层次：寄存器 > 缓存 > 内存 > 外存（越往上越快、越小、越贵）

## 3. 嵌入式系统

- 定义：具有**专用功能**的计算机系统（软件 + 硬件结合，资源有限）
- 特点：专用性强、资源受限（CPU / 内存 / 存储有限）、软硬件可裁剪
- 学习路径：开发板（软硬件结合）实战

## 4. 进程（Process）

### 4.1 定义
进程是具有一定独立功能的程序在一个数据集合上依次动态执行的过程，是**正在执行程序的实例**。

### 4.2 核心结论
**进程是操作系统分配资源的基本单位**（内存空间、CPU 使用权等都以进程为单位分配）。

### 4.3 四个特征
1. 动态性：进程有生命周期（创建 → 运行 → 终止）
2. 并发性：多个进程可以同时推进（宏观并行）
3. 独立性：进程是独立运行、独立获得资源的单位
4. 异步性：各进程按各自不可预知的速度推进

### 4.4 进程的组成
1. 指令（代码段）：程序代码
2. 数据：程序处理的数据
3. PCB（进程控制块，位于内核中）：**进程存在的唯一标志**
...
| 状态 | 动态（执行中） | 静态（磁盘上的文件） |
| 生命周期 | 创建到终止 | 不运行也永久存在 |
| 对应关系 | 一个程序可产生多个进程 | 一个程序对应一个文件 |

### 4.6 进程的状态
创建态 → 就绪态 → 运行态 → 结束态
- 创建态：PCB 刚创建，尚未就绪
- 就绪态：已具备运行条件，等待 CPU 调度
- 运行态：正在占用 CPU 执行
- 阻塞态：等待某事件（如 I/O）完成
- 结束态：运行完毕，释放资源（未回收时残留 PCB 即僵尸态）

![进程状态转换图](https://github.com/user-attachments/assets/cf993d49-c06f-4dee-abfe-d3ef2deca203)

## 5. 单核 CPU 如何实现多进程并发

- 核心机制：**时间片轮转（轮询）**——每个进程分到一个时间片，轮流占用 CPU，切换极快，宏观上表现为"同时"运行
- 进程调度算法：
  1. 先来先服务（FCFS）
  2. 短作业优先（SJF）
  3. 时间片轮转（RR）
...
1. fork() 一次调用，**返回两次**：
   ...
   - 父进程中返回子进程的 PID
   - 出错返回 -1
2. fork 之后，子进程会**复制一份父进程的指令和数据**，两个进程都会从 fork 处继续往下执行
3. **PCB 是单独独立创建的，不是复制**（PCB 是进程存在的唯一标志）
4. 进程 main 的父进程是 bash（由 shell 启动的程序都是 shell 的子进程）

### 踩坑点
1. 判断子进程用 `pid == 0`，别写成 `pid > 0`（那是父进程的分支）
2. fork 之后子进程不是从 main 开头执行，而是从 fork 语句的下一行开始

## 7. 线程（Thread）

- 定义：进程内部的**一个执行单元**，是系统独立调度和分派 CPU 的基本单位

### 进程 vs 线程

| 对比项 | 进程 | 线程 |
| --- | --- | --- |
| 角色 | 资源分配的基本单位 | CPU 调度 / 分派的基本单位 |
| 资源 | 独立拥有内存空间等资源 | 共享所属进程的资源 |
| 创建 / 切换开销 | 大 | 小 |
| 数量关系 | 一个进程可包含多个线程 | 线程从属于进程 |

## 8. 进程终止与回收：return / exit / wait / waitpid

经典 Unix/Linux 系统编程考点，也是面试常考的**进程生命周期**问题。

### 8.1 角色划分
1. `return` 和 `exit`：**进程 / 函数结束**的方式
2. `wait` / `waitpid`：**父进程回收子进程**的方式

### 8.2 return vs exit（结束方式）

最本质区别：**`return` 是语言层面的，`exit` 是系统层面的。**

| 对比项 | `return` | `exit()` |
| --- | --- | --- |
| 作用范围 | 仅结束**当前函数**，返回调用者 | 结束**整个进程**，直接返回给操作系统 |
| 执行时机 | main 中 return 等价于调用 exit（C 标准规定） | 任意函数中调用，立即终止进程 |
| 清理工作 | 只执行局部变量析构（C++），返回上一层 | 执行全局/静态对象析构、刷新缓冲区、关闭文件描述符，最后调用内核的 `_exit` |
| 缓冲区 | main 中 return 会刷新所有标准 I/O 缓冲区（printf 未换行的内容会输出） | `exit()` 刷新缓冲区（`_exit()` 不刷新） |

**核心结论**：main 函数里 `return 0` 和 `exit(0)` 效果几乎一样；但**子函数**里 `return` 只是返回上一层，`exit` 会直接杀掉整个进程。

### 8.3 wait / waitpid（回收机制）

`wait` 和 `waitpid` 是**父进程**使用的系统调用，用来**回收子进程**（防止僵尸进程）。

1. `wait(&status)`：阻塞父进程，直到**任意一个**子进程结束，然后回收它
2. `waitpid(pid, &status, options)`：指定回收**特定 PID** 的子进程，可通过 `WNOHANG` 选项实现**非阻塞**轮询

**与 exit / return 的关系**：子进程调用 `exit` 或 `return` 结束后，它的"尸体"（PCB）不会立即消失，而是变成**僵尸进程**，直到父进程调用 `wait` 或 `waitpid` 把它"收尸"才算彻底结束。
...
**流程解析**：
1. 子进程执行到 `return 0` 或 `exit(0)` 时，**不会立刻消失**
2. 此时子进程变成 **Zombie（僵尸）** 状态，退出码（exit code）被保存在内核的进程表中
3. 父进程调用 `waitpid`，内核把子进程的退出码传给父进程，然后彻底删除子进程的 PCB，子进程才真正消失

### 8.5 最容易混淆的 3 个高频考点

| 场景 | 现象 | 原因 |
| --- | --- | --- |
| 子函数里 `return` | 程序继续执行，不会退出 | `return` 只结束当前函数 |
| 子函数里 `exit` | 程序**立即终止**，不再执行后续代码 | `exit` 是进程级终止 |
| 子进程结束，父进程不 `wait` | 产生**僵尸进程**，`ps -ef` 状态为 `<defunct>` | 子进程的退出信息没人收集，内核无法释放 |

### 8.6 记忆口诀

- `return`：**"下班的员工"**——只离开当前工作岗，回到主管那里
- `exit`：**"辞职的员工"**——彻底离开公司，经过 HR 办手续（清理缓冲区）
- `wait` / `waitpid`：**"HR 回收工牌"**——父进程等待并回收子进程的退出状态，避免产生"幽灵员工"（僵尸进程）

### 8.7 实际开发建议

1. 子进程里用 `_exit()`（不刷新缓冲区）更高效，但容易导致 printf 内容丢失
2. 子进程里用 `exit()` 更安全，但会多一次系统调用
3. main 函数结束处 `return` 和 `exit` 都行，`exit` 更明确表示进程终止
